...
namespace Font {
	...
	bool Renderer::Init(size_t maxChar,const glm::vec2& screen){
		if (maxChar > (USHRT_MAX + 1) / 4) {
			std::cerr << "WARNIG:" << maxChar << "は設定可能な最大文字数を超えています" << std::endl;
	...
	bool Renderer::LoadFromFile(const char* filename) {
		...
		ret = fscanf(fp.get(),
			...
			&scale.x, &scale.y);
		if (ret < 2) {
			std::cerr << "ERROR" << filename << "読み込みに失敗(line" << line <<
				")" << std::endl;
			return false;
		}
	...
	bool Renderer::AddString(const glm::vec2& position, const char* str) {
		...
		for (const char* itr = str;*itr;++itr) {
			...
			if (font.id >= 0 && font.size.x && font.size.y) {
				...
				const glm::vec2 offsetdPos = pos + font.offset * scale;
				p[0].position = offsetdPos + glm::vec2(0, -size.y);
				p[0].uv = font.uv * 65535.0f;
				p[0].color = color;
				p[1].position = offsetdPos + glm::vec2(size.x, -size.y);
				p[1].uv = (font.uv + glm::vec2(font.size.x, 0)) * 65535.0f;
				p[1].color = color;
				p[2].position = offsetdPos + glm::vec2(size.x, 0);
				p[2].uv = (font.uv + font.size) * 65535.0f;
				p[2].color = color;
				p[3].position = offsetdPos;
