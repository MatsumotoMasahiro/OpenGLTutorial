...
*
*@return 作成にせいこうした場合はテクスチャポインタを返す
*			失敗した場合はnullptrを返す
*/
TexturePtr Texture::Create(
	int width, int heigh, GLenum iformat, GLenum format, const void* data)
{
	struct Impl : Texture {
		Impl() {}
		~Impl() {}
	...
	glTexImage2D(
		GL_TEXTURE_2D, 0, iformat, width, heigh, 0, format, GL_UNSIGNED_BYTE, data);
...
struct stat st;
	...
	const size_t bmpFileHeaderSize = 14;//ビットマップフィルヘッダの倍と数
	const size_t windowsV1HeaderSize = 40;//ビットマップ情報へのヘッダのバイト数
	...
	return Create(width, height, GL_RGBA8, GL_BGR, buf.data() + offsetBytes);
}
