...
namespace Mesh {
	...
	*/
	...
	bool FbxLoader::Load(const char* filename) {
		std::unique_ptr<FbxManager, Delete<FbxManager>> fbxManager(FbxManager::Create());
		if (!fbxManager) {
		...
		if (!fbxScene) {
			...
			std::unique_ptr<FbxImporter, Delete<FbxImporter>>fbxImporter(
	...
	*FBXデータを仮データに変関する
	*
	*@param fbxNode 変なkん対象のFBXノードへのポインタ
	*
	*@retval true 変換成功
	*@retval false 変換失敗
	*/
	bool FbxLoader::Convert(FbxNode* fbxNode) {
		...
		const int chilCount = fbxNode->GetChildCount();
	...
	*/
	bool FbxLoader::LoadMesh(FbxNode* fbxNode) {
		...
		//頂点要素の有無を調べる
		const bool hasColor = fbxMesh->GetElementVertexColor() > 0;
		...
		//色情報を読取る準備
		//@note座標/UV/っ法線以外のパラメータには直接読み取る関数が提供されていないたｍｗ
	 ...
	 *@param begin		描画するマテリアルの戦闘のインデックス
	 *@param end		描画するマテリアルの終端のインデックス
	 */
	...
	void Mesh::Draw(const BufferPtr& buffer)const {
		...
		if(buffer->GetMesh(name.c_str()).get() != this){
			std::cerr << "WARNIG: バッファに存在しないメッシュ" << name << "描画しようとしました" << std::endl;
			return;
		}
		...
		for(TemporaryMesh& e : loader.meshList){
			for (TemporaryMaterial& material : e.materialList) {
				...
				if (vboEnd + verticeBytes >= vboSize) {
					std::cerr << "WARING: VBO サイズが不足しています(" <<
				...
				if(iboEnd + indicesBytes >= iboSize){
					std::cerr << "WARING: IBO サイズが不足しています(" <<
						iboEnd << "/" << iboSize << ")" << std::endl;
					continue;
				}

				glBufferSubData(GL_ARRAY_BUFFER, vboEnd, verticeBytes,
